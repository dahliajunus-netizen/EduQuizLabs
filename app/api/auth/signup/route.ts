...
export async function POST(request: Request) {
  try {
    ...
    // Create the Auth account as already confirmed. No confirmation email is sent.
    ...
    if (!createResponse.ok || !createData?.id) {
      const message = String(
        createData?.msg || createData?.message || createData?.error_description || '',
      ).trim();
    ...
    // Sign the confirmed user in immediately and return the session to the browser.
    ...
    if (!tokenResponse.ok || !tokenData?.access_token || !tokenData?.user?.id) {
      ...
      return NextResponse.json(
        { error: 'Account was created, but automatic sign-in failed. Please sign in normally.' },
        { status: 500 },
      );
    }

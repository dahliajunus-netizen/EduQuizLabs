...
const safeTest = (test: any) => ({
  id: test.id, class_code: test.class_code, course_id: test.course_id, title: test.title,
  description: test.description, published: test.published, due_date: test.due_date,
  time_limit_minutes: test.time_limit_minutes, max_attempts: test.max_attempts,
  allow_review: test.allow_review, requires_password: Boolean(String(test.test_password || '').trim()),
});

const safeQuestions = (rows: any[]) => rows.map(q => ({
  id: q.id, test_id: q.test_id, question_order: q.question_order, question: q.question,
...
async function createAttempt(id: string, userId: string) {
  ...
  const rows = await supabaseDb('test_attempts', {
...
async function completeAttempt(attemptId: string, answers: Record<string, unknown>, testId: string, studentId: string) {
  const existing = await supabaseDb(`test_submissions?test_id=eq.${encodeURIComponent(testId)}&student_id=eq.${encodeURIComponent(studentId)}&select=id&limit=1`);
  if (Array.isArray(existing) && existing.some((row: any) => row.attempt_id === attemptId)) return null;
  const questions = await getQuestions(testId);
  const score = calculateScore(questions, answers);
  const created = await supabaseDb('test_submissions', {
    method: 'POST', headers: { 'Content-Type': 'application/json', Prefer: 'return=representation' },
    body: JSON.stringify({ test_id: testId, student_id: studentId, answers, score }),
  });
  const now = new Date().toISOString();
  await supabaseDb(`test_attempts?id=eq.${encodeURIComponent(attemptId)}&status=eq.in_progress`, {
  ...
  try {
    ...
    if (action === 'access') {
      ...
      if (expected) {
        ...
        if (assessmentPasswordRateLimited(key)) return NextResponse.json({ error: 'Too many password attempts. Please wait a minute and try again.' }, { status: 429 });
        ...
        if (result.legacy) {
          await supabaseDb(`tests?id=eq.${encodeURIComponent(id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ test_password: await hashAssessmentPassword(expected) }) });
        }
      }
      ...
      if (!attempt) attempt = await createAttempt(id, user.id);
    ...
    if (action === 'save') {
      if (expired(attempt, test)) {
        const result = await completeAttempt(attempt.id, answers, id, user.id);
    ...
    if (action === 'submit') {
      const result = expired(attempt, test) ? await completeAttempt(attempt.id, answers, id, user.id) : await completeAttempt(attempt.id, answers, id, user.id);
      if (!result) return NextResponse.json({ error: 'This attempt has already been submitted.' }, { status: 409 });

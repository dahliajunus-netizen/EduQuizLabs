...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const tests = await supabaseDb(`tests?id=eq.${encodeURIComponent(String(testId))}&select=id,class_code,course_id`);
    ...
    const updated = await supabaseDb(`tests?id=eq.${encodeURIComponent(String(testId))}`, {
      method: 'PATCH',

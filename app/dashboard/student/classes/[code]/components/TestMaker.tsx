...
export default function TestMaker({title,setTitle,description,setDescription,dueDate,setDueDate,questions,busy,message,onSaveDetails,onAddQuestion,onEditQuestion,onDeleteQuestion,onClose,pointsFor,testId}:Props){
 ...
 useEffect(()=>{
   ...
   let cancelled=false;
   setLoadedPassword(false);setPasswordMessage('');setOptionsMessage('');
   fetch(`${url}/rest/v1/tests?id=eq.${encodeURIComponent(testId)}&select=test_password,max_attempts,allow_review`,{headers,cache:'no-store'})
     .then(async r=>{if(!r.ok)throw new Error(await r.text());return r.json();})
     .then(rows=>{if(!cancelled){const row=rows?.[0]||{};setPassword(String(row.test_password??''));setMaxAttempts(String(Math.max(1,Number(row.max_attempts)||1)));setAllowReview(row.allow_review!==false);}})
     .catch(()=>{if(!cancelled)setPasswordMessage('Could not load the current test settings. You can still enter new settings and save them.');})
     .finally(()=>{if(!cancelled)setLoadedPassword(true);});
   return()=>{cancelled=true;};
 },[testId]);

 async function savePassword(){
   if(!testId){setPasswordMessage('This test has no ID yet. Close and reopen the Test Maker after creating the test.');return;}
   if(savingPassword)return;
   setSavingPassword(true);setPasswordMessage('');
   try{const p=await fetch(`${url}/rest/v1/tests?id=eq.${encodeURIComponent(testId)}`,{method:'PATCH',headers,body:JSON.stringify({test_password:password.trim()||null}),cache:'no-store'});if(!p.ok)throw new Error(await p.text());setPasswordMessage(password.trim()?'Test password saved.':'Test password removed.');}
   catch(e){setPasswordMessage(e instanceof Error?e.message:'Failed to save test password.');}
   finally{setSavingPassword(false);}
 }

 async function saveOptions(){
   if(!testId){setOptionsMessage('This test has no ID yet.');return;}
   const n=Number(maxAttempts);
   if(!Number.isInteger(n)||n<1||n>100){setOptionsMessage('Maximum attempts must be a whole number from 1 to 100.');return;}
   setSavingOptions(true);setOptionsMessage('');
   try{const p=await fetch(`${url}/rest/v1/tests?id=eq.${encodeURIComponent(testId)}`,{method:'PATCH',headers,body:JSON.stringify({max_attempts:n,allow_review:allowReview}),cache:'no-store'});if(!p.ok)throw new Error(await p.text());setOptionsMessage('Attempt and review settings saved.');}
   catch(e){setOptionsMessage(e instanceof Error?e.message:'Failed to save test settings.');}
 ...
 const optionsSuccess=optionsMessage.toLowerCase().includes('saved');
 ...
 return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 p-4"><div className="mx-auto my-6 w-full max-w-4xl"><Card><CardHeader className="flex flex-row items-start justify-between gap-4"><div><CardTitle>Test Maker</CardTitle><p className="mt-1 text-sm text-muted-foreground">Build the entire test here.</p></div><Button type="button" variant="ghost" size="sm" onClick={onClose}><X/></Button></CardHeader><CardContent className="space-y-6">
   ...
   <div className="rounded-lg border p-4"><div className="mb-3 flex items-center gap-2"><Lock className="size-4"/><div><p className="font-medium">Test Password <span className="text-xs font-normal text-muted-foreground">(optional)</span></p><p className="text-xs text-muted-foreground">Students must enter this before starting the test.</p></div></div><div className="flex flex-wrap gap-2"><Input className="min-w-0 flex-1" type={showPassword?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Leave empty for no password" disabled={!loadedPassword}/><Button type="button" variant="outline" size="icon" onClick={()=>setShowPassword(v=>!v)} disabled={!loadedPassword} title={showPassword?'Hide password':'Show password'}><span className="pointer-events-none">{showPassword?<EyeOff className="size-4"/>:<Eye className="size-4"/>}</span></Button><Button type="button" variant="default" onClick={savePassword} disabled={savingPassword||!testId} className="relative z-20 pointer-events-auto gap-2"><span className="pointer-events-none">{savingPassword?<Loader2 className="size-4 animate-spin"/>:<Save className="size-4"/>}</span><span>{savingPassword?'Saving...':'Save Password'}</span></Button></div>{!loadedPassword&&testId&&<p className="mt-2 text-xs text-muted-foreground">Loading current password…</p>}{!testId&&<p className="mt-2 text-xs text-muted-foreground">Create the test first before assigning a password.</p>}{passwordMessage&&<p className={`mt-2 text-sm ${passwordMessageIsSuccess?'text-green-600 dark:text-green-400':'text-destructive'}`}>{passwordMessage}</p>}</div>
   <div className="rounded-lg border p-4 space-y-4"><div><p className="font-medium">Student Attempts & Review</p><p className="text-xs text-muted-foreground">Control how many times each student may submit this test and whether they can review their answers.</p></div><div className="grid gap-4 md:grid-cols-2"><div className="space-y-2"><label className="text-sm font-medium">Maximum attempts</label><Input type="number" min={1} max={100} step={1} value={maxAttempts} onChange={e=>setMaxAttempts(e.target.value)} disabled={!loadedPassword||savingOptions}/><p className="text-xs text-muted-foreground">1 = one attempt. 3 = up to three attempts.</p></div><div className="flex items-center gap-3 rounded-md border p-3"><input id="allow-review" type="checkbox" className="size-4" checked={allowReview} onChange={e=>setAllowReview(e.target.checked)} disabled={!loadedPassword||savingOptions}/><label htmlFor="allow-review"><span className="font-medium">Allow test review</span><span className="block text-xs text-muted-foreground">Students can review their submitted attempt after grading.</span></label></div></div><Button type="button" variant="outline" onClick={saveOptions} disabled={savingOptions||!testId||!loadedPassword}>{savingOptions?<Loader2 className="mr-2 size-4 animate-spin"/>:<Save className="mr-2 size-4"/>}{savingOptions?'Saving...':'Save Attempt & Review Settings'}</Button>{optionsMessage&&<p className={`text-sm ${optionsSuccess?'text-green-600 dark:text-green-400':'text-destructive'}`}>{optionsMessage}</p>}</div>

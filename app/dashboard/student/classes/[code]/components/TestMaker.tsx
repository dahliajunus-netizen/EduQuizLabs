...
export default function TestMaker({title,setTitle,description,setDescription,dueDate,setDueDate,questions,busy,message,onSaveDetails,onAddQuestion,onEditQuestion,onDeleteQuestion,onClose,pointsFor,testId}:Props){
 ...
 return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 p-4"><div className="mx-auto my-6 w-full max-w-4xl"><Card><CardHeader className="flex flex-row items-start justify-between gap-4"><div><CardTitle>Test Maker</CardTitle><p className="mt-1 text-sm text-muted-foreground">Build the entire test here.</p></div><Button type="button" variant="ghost" size="sm" onClick={onClose}><X/></Button></CardHeader><CardContent className="space-y-6">
   ...
   <div className="rounded-lg border p-4"><div className="mb-3 flex items-center gap-2"><Lock className="size-4"/><div><p className="font-medium">Test Password <span className="text-xs font-normal text-muted-foreground">(optional)</span></p><p className="text-xs text-muted-foreground">Students must enter this before starting the test.</p></div></div><div className="flex gap-2"><Input type={showPassword?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Leave empty for no password" disabled={!loadedPassword}/><Button type="button" variant="outline" size="icon" onClick={()=>setShowPassword(v=>!v)} disabled={!loadedPassword}>{showPassword?<EyeOff className="size-4"/>:<Eye className="size-4"/>}</Button><Button type="button" onClick={savePassword} disabled={savingPassword||!testId||!loadedPassword}>{savingPassword?<Loader2 className="size-4 animate-spin"/>:<Save className="size-4"/>}</Button></div>{!testId&&<p className="mt-2 text-xs text-muted-foreground">Create/save the test first before assigning a password.</p>}{passwordMessage&&<p className="mt-2 text-sm text-muted-foreground">{passwordMessage}</p>}</div>

...
export default function ClassDetailsPage() {
  ...
  async function leaveClass() {
    if (teacher || !studentId || !code || busy) return;
    if (!confirm('Are you sure you want to leave this class?\n\nYou will need the class code to join again.')) return;
    setBusy(true);
    setMessage('');
    try {
      await del(`${url}/rest/v1/student_classes?code=eq.${encodeURIComponent(code)}&student_id=eq.${encodeURIComponent(studentId)}`);
      window.location.href = '/dashboard/student';
    } catch (e) {
      setMessage(e instanceof Error ? e.message : 'Failed to leave class.');
    } finally {
      setBusy(false);
    }
  }

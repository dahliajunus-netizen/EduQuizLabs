...
import { ArrowLeft, BookOpen, FileText, Loader2, PlusCircle, X } from 'lucide-react';
...
const emptyQuestion: QuestionFormState = {
  ...
  questionType: 'multiple-choice', correctAnswer: 'A',
};
...
export default function ClassDetailsPage() {
  ...
  async function patch(u: string, body: unknown) {
    const response = await fetch(u, { method: 'PATCH', headers: jsonHeaders, body: JSON.stringify(body) });
    if (!response.ok) throw new Error((await response.text()) || `Update failed (${response.status})`);
  }
    ...
    try {
      ...
      for (const course of cs) {
        ...
        for (const assignment of aa[course.id]) {
          ...
          const filter = teacher ? `assignment_id=eq.${encodeURIComponent(assignment.id)}` : `assignment_id=eq.${encodeURIComponent(assignment.id)}&student_id=eq.${encodeURIComponent(studentId)}`;
          ss[assignment.id] = await get<Submission[]>(`${url}/rest/v1/assignment_submissions?${filter}&select=*`).catch(() => []);
        }
      }
  ...
  async function deleteItem(table: string, id: string) {
    ...
    try { await del(`${url}/rest/v1/${table}?id=eq.${encodeURIComponent(id)}`); await load(); }
  ...
  async function createCourse(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`${url}/rest/v1/class_courses`, { method: 'POST', headers: jsonHeaders, body: JSON.stringify({ course_name: courseName.trim(), class_code: code }) });
  ...
  async function checkMaterialLink(value: string) {
    setLinkCheckStatus('checking'); setLinkCheckReason('Gemini is checking this link...');
    try {
      const response = await fetch('/api/moderate-link', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: value.trim() }) });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) { setLinkCheckStatus('error'); setLinkCheckReason(result?.reason || 'The link could not be checked.'); return false; }
      if (result?.safe === true) { setLinkCheckStatus('safe'); setLinkCheckReason(result.reason || 'Link passed the Gemini safety check.'); return true; }
      setLinkCheckStatus('unsafe'); setLinkCheckReason(result?.reason || 'This link is not allowed as classroom material.'); return false;
    } catch { setLinkCheckStatus('error'); setLinkCheckReason('The link could not be checked. Please try again.'); return false; }
  }
  ...
  async function addItem(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (addType === 'material') {
        ...
        if (!(await checkMaterialLink(materialLink))) return;
        const response = await fetch(`${url}/rest/v1/course_materials`, { method: 'POST', headers: jsonHeaders, body: JSON.stringify({ course_id: selectedCourse.id, name: materialName.trim(), link: materialLink.trim() }) });
        if (!response.ok) throw new Error(await response.text());
      } else {
        ...
        const response = await fetch(`${url}/rest/v1/course_assignments`, { method: 'POST', headers: jsonHeaders, body: JSON.stringify({ course_id: selectedCourse.id, name: assignmentName.trim(), description: assignmentDescription.trim(), due_date: due }) });
        if (!response.ok) throw new Error(await response.text());
      }
      setAddModal(false); setMaterialName(''); setMaterialLink(''); setAssignmentName(''); setAssignmentDescription(''); setAssignmentDueDate(''); setLinkCheckStatus('idle'); setLinkCheckReason(''); await load();
  ...
  async function createTest(course: Course) {
    ...
    setBusy(true); setMessage('');
    try {
      const response = await fetch(`${url}/rest/v1/tests`, { method: 'POST', headers: jsonHeaders, body: JSON.stringify({ course_id: course.id, title: 'Untitled Test', description: null, due_date: null, published: false }) });
      const text = await response.text();
      ...
      openBuilder(test as Test); await load();
    } catch (e) { alert(e instanceof Error ? e.message : 'Failed to create test.'); }
    finally { setBusy(false); }
  }

  function openBuilder(test: Test) {
    setBuilderTest(test); setTestTitle(test.title || ''); setTestDescription(test.description || ''); setTestDueDate(displayDate(test.due_date)); setMessage(''); setBuilderOpen(true);
  }

  async function saveTestDetails() {
    if (!builderTest || !testTitle.trim()) { setMessage('Test title is required.'); return; }
    ...
    } catch (e) { setMessage(e instanceof Error ? e.message : 'Failed to save test details.'); }
    finally { setBusy(false); }
  }

  function openNewQuestion() {
    if (!builderTest) return;
    setEditingQuestion(null); setQuestionForm({ ...emptyQuestion }); setMessage(''); setQuestionModal(true);
  }

  function openEditQuestion(question: Question) {
    setEditingQuestion(question);
    setQuestionForm({ questionText: question.question, optionA: question.option_a || '', optionB: question.option_b || '', optionC: question.option_c || '', optionD: question.option_d || '', questionType: question.question_type || 'multiple-choice', correctAnswer: question.correct_answer || 'A' });
    setMessage(''); setQuestionModal(true);
  }
  ...
  async function saveQuestion(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!f.questionText.trim()) { setMessage('Question text is required.'); return; }
    if (f.questionType === 'multiple-choice' && (!f.optionA.trim() || !f.optionB.trim() || !f.optionC.trim() || !f.optionD.trim())) { setMessage('Multiple choice questions require all four answers.'); return; }
    if ((f.questionType === 'fill-blank' || f.questionType === 'matching') && (!f.optionA.trim() || (f.questionType === 'matching' && !f.optionB.trim()))) { setMessage('Fill in the required answer fields.'); return; }
    setBusy(true); setMessage('');
    try {
      const body = {
        test_id: builderTest.id, question: f.questionText.trim(), question_type: f.questionType,
        option_a: f.questionType === 'true-false' ? 'True' : f.optionA.trim(),
        option_b: f.questionType === 'true-false' ? 'False' : f.questionType === 'matching' ? f.optionB.trim() : f.optionB.trim(),
        option_c: f.questionType === 'multiple-choice' ? f.optionC.trim() : '',
        option_d: f.questionType === 'multiple-choice' ? f.optionD.trim() : '',
        correct_answer: f.questionType === 'true-false' ? (f.correctAnswer === 'B' ? 'B' : 'A') : f.questionType === 'matching' ? 'B' : f.questionType === 'fill-blank' ? 'A' : f.correctAnswer,
        question_order: editingQuestion?.question_order ?? ((questions[builderTest.id]?.length || 0) + 1), points: 0,
      };
      if (editingQuestion?.id) await patch(`${url}/rest/v1/test_questions?id=eq.${encodeURIComponent(editingQuestion.id)}`, body);
      else { const response = await fetch(`${url}/rest/v1/test_questions`, { method: 'POST', headers: jsonHeaders, body: JSON.stringify(body) }); if (!response.ok) throw new Error(await response.text()); }
      await rebalanceQuestions(builderTest.id); setQuestionModal(false); setEditingQuestion(null); setQuestionForm({ ...emptyQuestion }); await load();
    } catch (e) { alert(e instanceof Error ? e.message : 'Failed to save question.'); }
    finally { setBusy(false); }
  }

  async function deleteQuestion(question: Question) {
    if (!question.id || !confirm('Delete this question?')) return;
    try { await del(`${url}/rest/v1/test_questions?id=eq.${encodeURIComponent(question.id)}`); await rebalanceQuestions(question.test_id); await load(); }
    catch { alert('Failed to delete question.'); }
  }

  async function deleteTest(test: Test) {
    if (!confirm(`Delete "${test.title}"?`)) return;
    try { await del(`${url}/rest/v1/tests?id=eq.${encodeURIComponent(test.id)}`); await load(); }
    catch { alert('Failed to delete test.'); }
  }

  async function toggleTest(test: Test) {
    if (!test.published && (!test.title.trim() || !test.due_date || !(questions[test.id] || []).length)) { alert('Finish the test in Test Maker first: add a title, due date, and at least one question.'); return; }
    try { await patch(`${url}/rest/v1/tests?id=eq.${encodeURIComponent(test.id)}`, { published: !test.published }); await load(); }
    catch { alert('Failed to change publication status.'); }
  }
  ...
  function openSubmit(assignment: Assignment) {
    ...
    setSelectedAssignment(assignment); setSubmissionClass(''); setSubmissionLink(''); setSubmissionModal(true);
  }

  async function submitAssignment(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!selectedAssignment?.id || !studentId || !submissionClass.trim() || !validUrl(submissionLink)) return;
    setBusy(true);
    try {
      const response = await fetch(`${url}/rest/v1/assignment_submissions`, { method: 'POST', headers: jsonHeaders, body: JSON.stringify({ assignment_id: selectedAssignment.id, student_id: studentId, nickname: name, class: submissionClass.toUpperCase(), link: submissionLink.trim(), grade: null }) });
      if (!response.ok) throw new Error(await response.text());
      setSubmissionModal(false); await load();
    } catch (e) { alert(e instanceof Error ? e.message : 'Submission failed.'); }
    finally { setBusy(false); }
  }

  async function undoSubmission(assignment: Assignment) {
    const submission = studentSubmission(assignment.id!);
    if (!submission?.id || !confirm('Undo your submission?')) return;
    try { await del(`${url}/rest/v1/assignment_submissions?id=eq.${encodeURIComponent(submission.id)}&student_id=eq.${encodeURIComponent(studentId)}`); await load(); }
    catch { alert('Failed to undo submission.'); }
  }

  async function saveGrade(submission: Submission) {
    if (!submission.id) return;
    const raw = Number(gradeInputs[submission.id] ?? submission.grade ?? 0);
    if (!Number.isFinite(raw) || raw < 0 || raw > 100) { alert('Grade must be between 0 and 100.'); return; }
    try { await patch(`${url}/rest/v1/assignment_submissions?id=eq.${encodeURIComponent(submission.id)}`, { grade: Math.trunc(raw) }); await load(); }
    ...
    <>
      ...
      <main className="container mx-auto space-y-8 px-6 py-8">
        ...
        <div className="flex items-end justify-between gap-4">
          <div><h1 className="text-3xl font-bold">{className}</h1><p className="text-sm text-muted-foreground">School: {school} · Code: <b className="font-mono text-primary">{code}</b></p></div>
          {teacher && <Button type="button" onClick={() => { setCourseName(''); setCourseError(''); setCourseModal(true); }}><PlusCircle className="mr-2 size-4" />Create New Course</Button>}
        </div>
      ...
      {builderOpen && builderTest && <TestMaker title={testTitle} setTitle={setTestTitle} description={testDescription} setDescription={setTestDescription} dueDate={testDueDate} setDueDate={setTestDueDate} questions={questions[builderTest.id] || []} busy={busy} message={message} onSaveDetails={saveTestDetails} onAddQuestion={openNewQuestion} onEditQuestion={q => openEditQuestion(q as Question)} onDeleteQuestion={q => q.id && deleteQuestion({ ...q, test_id: builderTest.id, question_order: 0, question_type: undefined })} onClose={() => { setBuilderOpen(false); setMessage(''); }} pointsFor={formatPoints} />}

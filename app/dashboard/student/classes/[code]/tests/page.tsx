...
export default function TestsPage() {
  ...
  const [submissions, setSubmissions] = useState<
    Record<string, TestSubmission | null>
  >({});
  ...
  /*
   * Load courses, tests and questions.
   */
  const load = async () => {
    ...
    try {
      const courseList = await getJson<Course[]>(
        `${supabaseUrl}/rest/v1/class_courses?class_code=eq.${encodeURIComponent(
          code
        )}&select=id,course_name,class_code&order=id.asc`
      );

      setCourses(courseList);

      if (!courseId && courseList[0]?.id) {
      ...
      const testList = await getJson<Test[]>(
        `${supabaseUrl}/rest/v1/tests?course_id=in.${encodeURIComponent(
          filter
        )}${teacher ? '' : '&published=eq.true'}&select=*&order=created_at.asc`
      );
      ...
      await Promise.all(
        testList.map(async (test) => {
          const select = teacher
            ? '*'
            : 'id,test_id,question,option_a,option_b,option_c,option_d,points,question_order';

          try {
            questionMap[test.id] = await getJson<Question[]>(
              `${supabaseUrl}/rest/v1/test_questions?test_id=eq.${encodeURIComponent(
                test.id
              )}&select=${select}&order=question_order.asc,id.asc`
            );
          } catch {
            questionMap[test.id] = [];
          }
          ...
          testList.map(async (test) => {
            try {
              const rows = await getJson<TestSubmission[]>(
                `${supabaseUrl}/rest/v1/test_submissions?test_id=eq.${encodeURIComponent(
                  test.id
  ...
  const courseName = (id: string) =>
    courses.find((c) => c.id === id)?.course_name || 'Course';

  /*
   * ADD QUESTION
   *
   * This is deliberately a normal synchronous state update.
  ...
  const addQuestion = () => {
    ...
    setDraftQuestions((previous) => [
      ...previous,
      {
        ...
        option_d: '',
        correct_answer: 'A',
        question_order: previous.length,
      },
    ]);
  };
   ...
   *
   * No points input is shown to teachers.
   */
  const automaticPoints =
    draftQuestions.length > 0
      ? 100 / draftQuestions.length
      : 0;
  ...
  const createTest = async (e: React.FormEvent) => {
    ...
    if (!teacher) {
      setError('You must be logged in as a teacher to create a test.');
      return;
    }
    ...
    const incomplete = draftQuestions.some(
      (q) =>
        !q.question.trim() ||
        !q.option_a.trim() ||
        !q.option_b.trim() ||
    ...
    if (incomplete) {
      setError(
        'Please complete the question and all four answer choices.'
      );
      return;
    }
    ...
    try {
      const createdRows = await postJson<Test[]>(
        `${supabaseUrl}/rest/v1/tests`,
        {
          course_id: courseId,
          title: title.trim(),
      ...
      const createdQuestions =
        await postJson<Question[]>(
          `${supabaseUrl}/rest/v1/test_questions`,
          rows
        );
      ...
      setError(
        err instanceof Error
          ? `Failed to create test: ${err.message}`
  ...
  const togglePublished = async (test: Test) => {
    ...
    try {
      const updatedRows = await patchJson<Test[]>(
        `${supabaseUrl}/rest/v1/tests?id=eq.${encodeURIComponent(
          test.id
        )}`,
        {
  ...
  const deleteTest = async (test: Test) => {
    if (
      ...
      !supabaseUrl ||
      !confirm(
        `Delete "${test.title}" and all of its questions?`
      )
    ) {
      return;
    }
  ...
  const toggleTest = async (test: Test) => {
    ...
    if (next && teacher && supabaseUrl) {
      try {
        const rows = await getJson<Question[]>(
          `${supabaseUrl}/rest/v1/test_questions?test_id=eq.${encodeURIComponent(
            test.id
  ...
  const startTest = (test: Test) => {
    ...
    if (submissions[test.id]) {
      setTeacherDetails(
        `You already submitted this test. Score: ${Number(
          submissions[test.id]?.score ?? 0
        ).toFixed(2)}%`
      );
      return;
    }
  ...
  const submitTest = async () => {
    ...
    if (
      !confirm(
        'Submit your test? You will not be able to submit it again.'
      )
    ) {
      return;
    }
    ...
    try {
      const rows =
        await postJson<TestSubmission[]>(
          `${supabaseUrl}/rest/v1/rpc/submit_test`,
          {
      ...
      setSubmissions((previous) => ({
        ...previous,
        [takingTest.id]: result,
      }));

      setTakingTest(null);

      setTeacherDetails(
        `Test submitted successfully. Your score is ${Number(
          result.score
        ).toFixed(2)}%.`
      );
    } catch (err) {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {teacher
                ? 'Create, publish, and review tests.'
                : 'Take your published tests and see your results.'}
            </p>
          </div>
        </div>
                ...
                <div className="space-y-4">
                  ...
                  {draftQuestions.length === 0 && (
                    <div className="rounded-lg border border-dashed p-5 text-center text-sm text-muted-foreground">
                      ...
                      to create your first multiple-choice question.
                    </div>
                  )}
              ...
              <CardContent className="py-12 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {teacher
                    ...
                    : 'Your teacher has not published a test for this class.'}
                </p>
              </CardContent>
            </Card>
          )}

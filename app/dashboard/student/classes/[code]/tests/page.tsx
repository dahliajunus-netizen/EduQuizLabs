...
export default function TestsPage() {
  ...
  const groupedTests = useMemo(
    () => courses.map((course) => ({ course, tests: tests.filter((t) => t.course_id === course.id) })).filter((group) => teacher || group.tests.length),
              ...
              <form onSubmit={createTest} className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div><h3 className="font-semibold">Questions ({draftQuestions.length})</h3><p className="text-xs text-muted-foreground">Every question is automatically worth {automaticPoints ? automaticPoints.toFixed(2) : '0.00'} points.</p></div>
                  ...
                  {draftQuestions.map((q, index) => (
                    <div key={index} className="space-y-5 rounded-xl border p-5">
                      <div className="flex items-start justify-between gap-3"><div><p className="font-semibold">Question {index + 1}</p><p className="text-xs text-muted-foreground">Automatically worth {automaticPoints.toFixed(2)} points.</p></div><Button type="button" variant="ghost" size="sm" onClick={() => removeDraft(index)} className="text-muted-foreground hover:text-destructive"><Trash2 className="size-4" /></Button></div>

                      <div className="rounded-lg border bg-muted/20 p-4">
                        <label className="mb-2 block text-sm font-semibold">Question Type</label>
                        <select value={q.type} onChange={(e) => changeQuestionType(index, e.target.value as QuestionType)} className="h-11 w-full rounded-md border-2 border-primary/30 bg-background px-3 text-sm font-medium outline-none focus:border-primary" aria-label={`Question ${index + 1} type`}>
                      ...
                      {q.type === 'multiple_choice' && (
                        <>
                          <div className="grid gap-3 md:grid-cols-2">
                            <Input value={q.option_a} onChange={(e) => updateDraft(index, { option_a: e.target.value })} placeholder="A:" required />
                            <Input value={q.option_b} onChange={(e) => updateDraft(index, { option_b: e.target.value })} placeholder="B:" required />
                            <Input value={q.option_c} onChange={(e) => updateDraft(index, { option_c: e.target.value })} placeholder="C:" required />
                            <Input value={q.option_d} onChange={(e) => updateDraft(index, { option_d: e.target.value })} placeholder="D:" required />
                          </div>
                          <div><label className="mb-2 block text-sm font-medium">Correct answer</label><select value={q.correct_answer || 'A'} onChange={(e) => updateDraft(index, { correct_answer: e.target.value })} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option></select></div>
                        </>
                      )}
                      ...
                      {q.type === 'true_false' && (
                        <div>
                          ...
                          <p className="mt-2 text-xs text-muted-foreground">Students will automatically receive only True and False as the two answer choices.</p>
                        </div>
                      )}
                    </div>
                  ))}

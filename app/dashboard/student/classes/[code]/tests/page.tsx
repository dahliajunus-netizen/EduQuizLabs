...
export default function TestsPage() {
  ...
  const updateDraft = (index: number, patch: Partial<DraftQuestion>) => {
    setDraftQuestions((previous) =>
      previous.map((q, i) => (i === index ? { ...q, ...patch } : q)),
    );
  };
  ...
  const changeQuestionType = (index: number, type: QuestionType) => {
    const defaults: Partial<DraftQuestion> = {
      type,
      correct_answer: type === 'multiple_choice' || type === 'true_false' ? 'A' : '',
    };
    updateDraft(index, defaults);
  };
  ...
  const updateMatchingPair = (qIndex: number, pairIndex: number, key: 'left' | 'right', value: string) => {
    setDraftQuestions((previous) => previous.map((q, i) => {
      ...
      return {
        ...q,
        matching_pairs: q.matching_pairs.map((pair, p) =>
          p === pairIndex ? { ...pair, [key]: value } : pair,
        ),
      };
    }));
  };
  ...
  const createTest = async (e: React.FormEvent) => {
    ...
    for (const [i, q] of draftQuestions.entries()) {
      ...
      if (q.type === 'fill_blank' && !q.fill_answers.some((v) => v.trim())) {
        return setError(`Question ${i + 1}: enter at least one accepted answer.`);
      }
      if (q.type === 'matching' && q.matching_pairs.some((p) => !p.left.trim() || !p.right.trim())) {
    ...
    try {
      ...
      const rows = draftQuestions.map((q, i) => {
        ...
        if (q.type === 'true_false') {
          optionA = 'True';
          optionB = 'False';
          correct = q.correct_answer === 'B' ? 'B' : 'A';
        }
        if (q.type === 'fill_blank') {
          correct = '';
          answerData = { acceptedAnswers: q.fill_answers.filter((v) => v.trim()).map((v) => v.trim()) };
        }
        if (q.type === 'matching') {
          correct = '';
          answerData = { pairs: q.matching_pairs.filter((p) => p.left.trim() && p.right.trim()) };
        }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto space-y-8 px-6 py-8">
        <div>
          <Link href={`/dashboard/${teacher ? 'teacher' : 'student'}/classes/${encodeURIComponent(code)}`}>
            <Button variant="ghost" className="-ml-3 mb-3 gap-2">
              <ArrowLeft className="size-4" /> Back to Class
            </Button>
          </Link>
          <h1 className="text-3xl font-bold">Tests</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {teacher ? 'Create, publish, and review tests.' : 'Take your published tests and see your results.'}
          </p>
        </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <PlusCircle className="size-5 text-primary" /> Create New Test
              </CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={createTest} className="space-y-6">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Chapter 3 Quiz" required />
                  </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="Optional instructions" className="w-full resize-none rounded-md border border-input bg-background px-3 py-2 text-sm" />
                  </div>
                  <div>
                    <label className="mb-2 block text-sm font-medium">Due date (optional)</label>
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <h3 className="font-semibold">Questions ({draftQuestions.length})</h3>
                      <p className="text-xs text-muted-foreground">Every question is automatically worth {automaticPoints ? automaticPoints.toFixed(2) : '0.00'} points.</p>
                    </div>
                    <Button type="button" variant="outline" onClick={addQuestion} disabled={creating} className="gap-2">
                  ...
                  {draftQuestions.map((q, index) => (
                    <div key={index} className="space-y-5 rounded-xl border p-5">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="font-semibold">Question {index + 1}</p>
                          <p className="text-xs text-muted-foreground">Automatically worth {automaticPoints.toFixed(2)} points.</p>
                        </div>
                        <Button type="button" variant="ghost" size="sm" onClick={() => removeDraft(index)} className="text-muted-foreground hover:text-destructive">
                      ...
                      <div className="rounded-lg border bg-muted/20 p-4">
                        ...
                        <select
                          ...
                          className="h-11 w-full rounded-md border-2 border-primary/30 bg-background px-3 text-sm font-medium outline-none focus:border-primary"
                        ...
                        <p className="mt-2 text-xs text-muted-foreground">Choose the format for this question. The fields below change automatically.</p>
                      </div>
                      ...
                      {q.type === 'multiple_choice' && (
                        <>
                          <div className="grid gap-3 md:grid-cols-2">
                            <Input value={q.option_a} onChange={(e) => updateDraft(index, { option_a: e.target.value })} placeholder="Option A" required />
                            <Input value={q.option_b} onChange={(e) => updateDraft(index, { option_b: e.target.value })} placeholder="Option B" required />
                            <Input value={q.option_c} onChange={(e) => updateDraft(index, { option_c: e.target.value })} placeholder="Option C" required />
                            <Input value={q.option_d} onChange={(e) => updateDraft(index, { option_d: e.target.value })} placeholder="Option D" required />
                          </div>
                          <div>
                            <label className="mb-2 block text-sm font-medium">Correct answer</label>
                            <select value={q.correct_answer || 'A'} onChange={(e) => updateDraft(index, { correct_answer: e.target.value })} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                              {['A', 'B', 'C', 'D'].map((x) => <option key={x} value={x}>Correct answer: {x}</option>)}
                            </select>
                          </div>
                        </>
                      )}
                      ...
                      {q.type === 'fill_blank' && (
                        <div className="space-y-3">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">Add answers that should all be accepted. Matching is case-insensitive.</p>
                          </div>
                          {q.fill_answers.map((answer, answerIndex) => (
                            <div key={answerIndex} className="flex gap-2">
                              <Input value={answer} onChange={(e) => updateDraft(index, { fill_answers: q.fill_answers.map((a, i) => i === answerIndex ? e.target.value : a) })} placeholder={`Accepted answer ${answerIndex + 1}`} />
                              {q.fill_answers.length > 1 && <Button type="button" variant="ghost" onClick={() => updateDraft(index, { fill_answers: q.fill_answers.filter((_, i) => i !== answerIndex) })}><Trash2 className="size-4" /></Button>}
                            </div>
                          ))}
                          <Button type="button" variant="outline" onClick={() => updateDraft(index, { fill_answers: [...q.fill_answers, ''] })}>Add accepted answer</Button>
                        </div>
                      )}
                      ...
                      {q.type === 'matching' && (
                        <div className="space-y-3">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">Each left item gets one correct right-side match.</p>
                          </div>
                          {q.matching_pairs.map((pair, pairIndex) => (
                            <div key={pairIndex} className="grid gap-2 md:grid-cols-[1fr_1fr_auto]">
                              <Input value={pair.left} onChange={(e) => updateMatchingPair(index, pairIndex, 'left', e.target.value)} placeholder={`Left item ${pairIndex + 1}`} />
                              <Input value={pair.right} onChange={(e) => updateMatchingPair(index, pairIndex, 'right', e.target.value)} placeholder={`Correct match ${pairIndex + 1}`} />
                              {q.matching_pairs.length > 1 && <Button type="button" variant="ghost" onClick={() => updateDraft(index, { matching_pairs: q.matching_pairs.filter((_, i) => i !== pairIndex) })}><Trash2 className="size-4" /></Button>}
                            </div>
                          ))}
                          <Button type="button" variant="outline" onClick={() => updateDraft(index, { matching_pairs: [...q.matching_pairs, { left: '', right: '' }] })}>Add pair</Button>
                        </div>
                      )}
                    </div>
                  ))}
                ...
                <Button type="submit" disabled={creating || !courseId || !title.trim() || !draftQuestions.length} className="gap-2">
                      ...
                      <div className="flex shrink-0 flex-wrap justify-end gap-2">
                        {teacher ? (
                          <>
                            ...
                            <Button type="button" size="sm" variant="ghost" onClick={() => setOpen((p) => ({ ...p, [test.id]: !p[test.id] }))}>{open[test.id] ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}</Button>
                          ...
                          <Button type="button" onClick={() => startTest(test)} disabled={!!submission}>{submission ? 'Completed' : 'Take Test'}</Button>
                        )}
                      </div>
                    </CardHeader>

                    {teacher && open[test.id] && (
                      <CardContent className="border-t pt-5">
                        <div className="space-y-3">
                          {!qs.length ? <p className="text-sm text-muted-foreground">No questions found.</p> : qs.map((q, i) => {
                            const type = q.question_type || 'multiple_choice';
                            return <div key={q.id || i} className="rounded-lg border p-4"><p className="font-medium">{i + 1}. {q.question}</p><p className="mt-1 text-xs font-medium text-primary">{TYPE_LABELS[type]}</p>{type === 'multiple_choice' && <div className="mt-3 grid gap-2 text-sm sm:grid-cols-2"><div>A. {q.option_a}</div><div>B. {q.option_b}</div><div>C. {q.option_c}</div><div>D. {q.option_d}</div></div>}{type === 'true_false' && <p className="mt-3 text-sm">True / False · Correct: {q.correct_answer === 'A' ? 'True' : 'False'}</p>}{type === 'fill_blank' && <p className="mt-3 text-sm">Accepted: {(q.answer_data?.acceptedAnswers || []).join(', ')}</p>}{type === 'matching' && <div className="mt-3 space-y-1 text-sm">{(q.answer_data?.pairs || []).map((pair, p) => <div key={p}>{pair.left} → {pair.right}</div>)}</div>}</div>;
                          })}
                        </div>
                      </CardContent>
                    )}
      ...
      {(message || error) && message && (
        <div className="fixed bottom-5 right-5 z-[80] max-w-sm rounded-xl border bg-card p-4 shadow-xl">
          <div className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-primary" /><p className="text-sm">{message}</p><button type="button" onClick={() => setMessage(null)}><X className="size-4" /></button></div>
        </div>
      )}

      {takingTest && (
        <div className="fixed inset-0 z-[70] overflow-y-auto bg-black/50 p-4 backdrop-blur-sm">
          <div className="mx-auto my-8 w-full max-w-3xl rounded-xl border bg-card p-6 shadow-2xl">
            <div className="mb-6 flex items-start justify-between gap-4">
              <div><h2 className="text-xl font-bold">{takingTest.title}</h2>{takingTest.description && <p className="mt-1 text-sm text-muted-foreground">{takingTest.description}</p>}{studentName && <p className="mt-2 text-xs text-muted-foreground">Student: {studentName}</p>}</div>
              <Button type="button" variant="ghost" size="sm" onClick={() => setTakingTest(null)} disabled={submitting}><X className="size-5" /></Button>
            </div>
            ...
            <div className="space-y-5">
              {(questions[takingTest.id] || []).map((q, i) => {
                ...
                const type = q.question_type || 'multiple_choice';
                const answer = testAnswers[id] || '';
                return <div key={id} className="rounded-xl border p-5">
                  ...
                  {type === 'true_false' && <div className="mt-4 grid gap-2 sm:grid-cols-2">{[['A','True'],['B','False']].map(([value, label]) => <label key={value} className={`flex cursor-pointer items-center gap-3 rounded-lg border p-3 ${answer === value ? 'border-primary bg-primary/5' : ''}`}><input type="radio" name={`question-${id}`} checked={answer === value} onChange={() => setTestAnswers((p) => ({ ...p, [id]: value }))} /><span>{label}</span></label>)}</div>}
                  {type === 'fill_blank' && <div className="mt-4"><Input value={answer} onChange={(e) => setTestAnswers((p) => ({ ...p, [id]: e.target.value }))} placeholder="Type your answer" autoComplete="off" /></div>}
                  {type === 'matching' && <div className="mt-4 space-y-3">{(q.answer_data?.pairs || []).map((pair, p) => { let current: Record<string,string> = {}; try { current = JSON.parse(answer || '{}'); } catch {} const rights = (q.answer_data?.pairs || []).map((x) => x.right); return <div key={p} className="grid gap-2 md:grid-cols-2"><div className="rounded-md border bg-muted/20 px-3 py-2 text-sm">{pair.left}</div><select value={current[pair.left] || ''} onChange={(e) => setTestAnswers((prev) => ({ ...prev, [id]: JSON.stringify({ ...current, [pair.left]: e.target.value }) }))} className="h-10 rounded-md border border-input bg-background px-3 text-sm"><option value="">Choose a match</option>{rights.map((right, r) => <option key={r} value={right}>{right}</option>)}</select></div>; })}</div>}
                </div>;
              })}
            </div>

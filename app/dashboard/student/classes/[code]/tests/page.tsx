...
import { Loader2, ArrowLeft, PlusCircle, Trash2, Save, CheckCircle2, X, ChevronUp, ChevronDown, ClipboardList, BookOpen } from 'lucide-react';

type QuestionType = 'multiple_choice' | 'true_false';

type Course = { id: string; course_name: string; class_code: string };
type Test = { id: string; course_id: string; title: string; description?: string | null; due_date?: string | null; published?: boolean | null; created_at?: string };
...
const emptyDraft = (): DraftQuestion => ({
  type: 'multiple_choice', question: '', option_a: '', option_b: '', option_c: '', option_d: '', correct_answer: 'A',
});
...
async function patchJson<T>(url: string, body: unknown): Promise<T> {
  const response = await fetch(url, { method: 'PATCH', headers: { 'Content-Type': 'application/json', Accept: 'application/json' }, body: JSON.stringify(body) });
  if (!response.ok) throw new Error(await response.text());
  return response.json();
}
...
export default function TestsPage() {
  ...
  useEffect(() => {
    try {
      ...
      setStudentName(String(data?.name || '').trim());
    } catch {}
  }, []);
  ...
  const load = async () => {
    ...
    try {
      ...
      await Promise.all(testList.map(async (test) => {
        try { questionMap[test.id] = await getJson<Question[]>(`${supabaseUrl}/rest/v1/test_questions?test_id=eq.${encodeURIComponent(test.id)}&select=*&order=question_order.asc,id.asc`); } catch { questionMap[test.id] = []; }
      }));
      setQuestions(questionMap);
      if (!teacher && studentId) {
        const submissionMap: Record<string, TestSubmission | null> = {};
        await Promise.all(testList.map(async (test) => {
          try { const rows = await getJson<TestSubmission[]>(`${supabaseUrl}/rest/v1/test_submissions?test_id=eq.${encodeURIComponent(test.id)}&student_id=eq.${encodeURIComponent(studentId)}&select=*&limit=1`); submissionMap[test.id] = rows[0] || null; } catch { submissionMap[test.id] = null; }
        }));
  ...
  const addQuestion = () => { setError(null); setDraftQuestions((previous) => [...previous, emptyDraft()]); };
  const updateDraft = (index: number, patch: Partial<DraftQuestion>) => setDraftQuestions((previous) => previous.map((q, i) => (i === index ? { ...q, ...patch } : q)));
  const changeQuestionType = (index: number, type: QuestionType) => type === 'true_false'
    ? updateDraft(index, { type, option_a: 'True', option_b: 'False', option_c: '', option_d: '', correct_answer: 'A' })
    : updateDraft(index, { type, option_a: '', option_b: '', option_c: '', option_d: '', correct_answer: 'A' });
  const removeDraft = (index: number) => setDraftQuestions((previous) => previous.filter((_, i) => i !== index));

  // test_questions.points is SMALLINT, so every stored point value MUST be an integer.
  // Distribute 100 points exactly: the first remainder questions receive one extra point.
  ...
  const createTest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rows = draftQuestions.map((q, i) => ({
        ...
        option_a: q.type === 'true_false' ? 'True' : q.option_a.trim(),
        option_b: q.type === 'true_false' ? 'False' : q.option_b.trim(),
        option_c: q.type === 'true_false' ? '' : q.option_c.trim(),
        option_d: q.type === 'true_false' ? '' : q.option_d.trim(),
        correct_answer: q.correct_answer, points: points[i], question_type: q.type, answer_data: {},
      }));
      ...
      setTitle(''); setDescription(''); setDueDate(''); setDraftQuestions([]); setMessage('Test created successfully.');
  ...
  const togglePublished = async (test: Test) => {
    ...
    try { const updatedRows = await patchJson<Test[]>(`${supabaseUrl}/rest/v1/tests?id=eq.${encodeURIComponent(test.id)}`, { published: !test.published }); const updated = updatedRows[0] || { ...test, published: !test.published }; setTests((previous) => previous.map((t) => t.id === test.id ? updated : t)); }
  ...
  const deleteTest = async (test: Test) => {
    ...
    try { await deleteFrom(`${supabaseUrl}/rest/v1/tests?id=eq.${encodeURIComponent(test.id)}`); setTests((previous) => previous.filter((t) => t.id !== test.id)); setQuestions((previous) => { const next = { ...previous }; delete next[test.id]; return next; }); }
  ...
  const submitTest = async () => {
    ...
    try {
      ...
      setSubmissions((previous) => ({ ...previous, [takingTest.id]: result })); setTakingTest(null); setMessage(`Test submitted successfully. Your score is ${Number(result.score).toFixed(2)}%.`);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto space-y-8 px-6 py-8">
        ...
        {teacher && <Card><CardHeader><CardTitle>Create Test</CardTitle></CardHeader><CardContent><form onSubmit={createTest} className="space-y-5">
          ...
          <div><label className="mb-2 block text-sm font-medium">Due date</label><Input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} /></div>
          <div className="space-y-3"><div className="flex items-center justify-between"><div><h3 className="font-semibold">Questions</h3><p className="text-xs text-muted-foreground">Points are automatically distributed to total exactly 100.</p></div><Button type="button" variant="outline" onClick={addQuestion} className="gap-2"><PlusCircle className="size-4" /> Add Question</Button></div>
            ...
            {draftQuestions.map((q, index) => <div key={index} className="rounded-lg border p-4"><div className="mb-4 flex items-center justify-between"><h4 className="font-semibold">Question {index + 1}</h4><Button type="button" variant="ghost" size="sm" onClick={() => removeDraft(index)} className="text-muted-foreground hover:text-destructive"><Trash2 className="size-4" /></Button></div><div className="space-y-4">
              ...
              {q.type === 'true_false' && <div><label className="mb-2 block text-sm font-medium">Correct answer</label><select value={q.correct_answer || 'A'} onChange={(e) => updateDraft(index, { correct_answer: e.target.value })} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="A">True</option><option value="B">False</option></select><p className="mt-2 text-xs text-muted-foreground">Students will automatically receive only True and False as the two answer choices.</p></div>}

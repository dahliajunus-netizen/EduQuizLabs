...
import { Loader2, ArrowLeft, PlusCircle, Trash2, Save, CheckCircle2, X, ChevronUp, ChevronDown, ClipboardList, BookOpen, Lock, Timer } from 'lucide-react';
  ...
  return <div className="min-h-screen bg-background">
    ...
    <main className="container mx-auto space-y-8 px-6 py-8">
      ...
      {teacher && <Card><CardHeader><CardTitle>Create Test</CardTitle></CardHeader><CardContent><form onSubmit={createTest} className="space-y-5">
        ...
        <div className="grid gap-4 md:grid-cols-3"><div><label className="mb-2 block text-sm font-medium">Due date</label><Input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} /></div><div><label className="mb-2 flex items-center gap-2 text-sm font-medium"><Lock className="size-4" /> Test password <span className="font-normal text-muted-foreground">(optional)</span></label><Input type="password" value={testPassword} onChange={e => setTestPassword(e.target.value)} placeholder="Leave empty for no password" /></div><div><label className="mb-2 flex items-center gap-2 text-sm font-medium"><Timer className="size-4" /> Time limit <span className="font-normal text-muted-foreground">(optional)</span></label><div className="flex items-center gap-2"><Input type="number" min="1" max="1440" step="1" value={timeLimitMinutes} onChange={e => setTimeLimitMinutes(e.target.value)} placeholder="30" /><span className="text-sm text-muted-foreground">minutes</span></div><p className="mt-1 text-xs text-muted-foreground">Starts when the student enters the test password, or clicks Take Test if there is no password.</p></div></div>

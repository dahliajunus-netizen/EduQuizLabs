...
import { Check, Clock, Loader2, Trophy, Wifi, Zap, ArrowLeft, Lock, Sparkles, X } from 'lucide-react';
...
export default function LiveQuizGame(){
  ...
  useEffect(()=>{
    ...
    setAnswered(false);setAnswer('');setAnswerCorrect(null);setSubmitting(false);setRemaining(30);
  ...
  async function submit(value:string){
    ...
    }catch(e){setAnswered(false);setAnswerCorrect(null);setSubmitting(false);setError(e instanceof Error?e.message:'Could not submit answer.');}
  }
  ...
  if(quiz.status==='finished')return <main className="min-h-screen bg-[radial-gradient(circle_at_top,hsl(var(--primary)/.12),transparent_50%)] px-4 py-8 sm:py-12"><div className="mx-auto max-w-2xl"><Card className="overflow-hidden rounded-[2rem] border-0 shadow-2xl"><CardHeader className="bg-primary/5 p-8 text-center sm:p-10"><div className="mx-auto flex size-20 items-center justify-center rounded-3xl bg-primary/10"><Trophy className="size-10 text-primary"/></div><p className="mt-5 text-[11px] font-black uppercase tracking-[0.3em] text-primary">Game complete</p><CardTitle className="mt-2 text-4xl font-black tracking-tight">Quiz Finished!</CardTitle><div className="mx-auto mt-5 grid max-w-sm grid-cols-2 gap-3"><div className="rounded-2xl border bg-card p-4"><p className="text-2xl font-black">{player.correct_answers}</p><p className="text-xs text-muted-foreground">Correct</p></div><div className="rounded-2xl border bg-card p-4"><p className="text-2xl font-black tabular-nums">{player.total_response_time_ms > 0 ? `${(player.total_response_time_ms / 1000).toFixed(2)}s` : "—"}</p><p className="text-xs text-muted-foreground">Time</p></div></div></CardHeader><CardContent className="p-5 sm:p-6"><div className="mb-4 flex items-center justify-between"><p className="text-sm font-black">Final leaderboard</p><p className="text-xs text-muted-foreground">{ranking.length} players</p></div><div className="space-y-2">{ranking.map((p,i)=><div key={p.id} className={`flex items-center gap-3 rounded-2xl border p-4 transition-all duration-300 ${p.id===player.id?'border-primary bg-primary/10 shadow-sm':''}`}><span className={`flex size-9 items-center justify-center rounded-xl font-black ${i<3?'bg-primary/10 text-primary':'bg-muted'}`}>{i+1}</span><span className="flex-1 truncate font-bold">{p.nickname}{p.id===player.id&&<span className="ml-2 rounded-full bg-primary/10 px-2 py-0.5 text-[10px] text-primary">YOU</span>}</span><div className="flex items-center gap-4 text-right"><div><p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Correct</p><p className="text-sm font-black">{p.correct_answers}</p></div><div><p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Time</p><p className="text-sm font-black tabular-nums">{p.total_response_time_ms > 0 ? `${(p.total_response_time_ms / 1000).toFixed(2)}s` : "—"}</p></div></div></div>)}</div><Button className="mt-6 h-11 w-full rounded-xl" onClick={()=>router.push('/dashboard/student')}><ArrowLeft className="mr-2 size-4"/>Back to Student Dashboard</Button></CardContent></Card></div></main>;
  ...
  if(revealing)return <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-5"><div className={`absolute inset-0 transition-all duration-500 ${answerCorrect===true?'bg-green-500/10':answerCorrect===false?'bg-red-500/10':'bg-primary/5'}`}/><div className="relative w-full max-w-3xl text-center animate-in fade-in zoom-in-95 duration-500"><div className={`mx-auto flex size-20 items-center justify-center rounded-3xl shadow-xl animate-in zoom-in duration-300 ${answerCorrect===true?'bg-green-500/15 text-green-600 dark:text-green-400':answerCorrect===false?'bg-red-500/15 text-red-600 dark:text-red-400':'bg-primary/10 text-primary'}`}>{answerCorrect===true?<Check className="size-11 animate-in zoom-in duration-300"/>:answerCorrect===false?<X className="size-11 animate-in zoom-in duration-300"/>:<Sparkles className="size-9 animate-pulse"/>}</div><p className={`mt-5 text-xs font-black uppercase tracking-[0.3em] ${answerCorrect===true?'text-green-600 dark:text-green-400':answerCorrect===false?'text-red-600 dark:text-red-400':'text-primary'}`}>{answerCorrect===true?'Correct!':answerCorrect===false?'Not quite':'Get ready'}</p><div className="mt-2 text-4xl font-black leading-tight tracking-tight sm:text-6xl animate-in fade-in slide-in-from-bottom-2 duration-500">{answerCorrect===true?'Great job!':answerCorrect===false?'Better luck next time!':'Get ready!'}</div><div className="mx-auto mt-7 rounded-[2rem] border bg-card/90 p-6 shadow-xl backdrop-blur"><Lock className="mx-auto size-7 text-primary"/><p className="mt-3 font-black">Next question coming up</p><p className="mt-1 text-sm text-muted-foreground">Use this short break to get ready.</p><div className="mt-5 grid grid-cols-4 gap-2">{shapes.map(s=><div key={s.letter} className={`flex h-14 items-center justify-center rounded-2xl ${s.className} text-2xl font-black text-white opacity-30`}>{s.symbol}</div>)}</div></div></div></main>;
  const progress=Math.max(0,Math.min(100,(remaining/30)*100));
  return <main className="min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,hsl(var(--primary)/.11),transparent_48%)] px-3 py-3 sm:px-5 sm:py-4"><div className="mx-auto flex min-h-[calc(100vh-1.5rem)] max-w-4xl flex-col"><div className="mb-3 flex shrink-0 items-center justify-between rounded-2xl border bg-card/90 px-4 py-2.5 shadow-lg backdrop-blur-xl sm:px-5"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">Live Quiz</p><p className="text-sm font-black">Question {currentIndex+1} <span className="text-muted-foreground">/ {questions.length}</span></p></div><div className={`relative flex min-w-20 items-center justify-center gap-2 overflow-hidden rounded-xl px-3 py-1.5 text-xl font-black transition-all duration-300 ${remaining<=5?'bg-destructive/10 text-destructive animate-pulse':'bg-primary/10 text-primary'}`}><div className="absolute inset-y-0 left-0 bg-current opacity-[0.1] transition-[width] duration-100" style={{width:`${progress}%`}}/><Clock className="relative size-5"/><span className="relative tabular-nums">{remaining}s</span></div></div><Card className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-[2rem] border-0 bg-card/95 shadow-2xl"><CardHeader className="shrink-0 px-5 pb-2 pt-4 text-center sm:px-8 sm:pt-5"><div className="mx-auto inline-flex items-center gap-2 rounded-full bg-green-500/10 px-3 py-1 text-[10px] font-black tracking-wider text-green-700 dark:text-green-300"><span className="size-1.5 animate-pulse rounded-full bg-green-500"/>ANSWERS ARE OPEN</div><CardTitle className="mt-2 text-lg sm:text-2xl">{currentQ.question}</CardTitle><p className="mt-1 text-xs text-muted-foreground">Tap once — your answer locks immediately.</p></CardHeader><CardContent className="flex min-h-0 flex-1 flex-col p-3 sm:p-4"><div className="grid min-h-0 flex-1 grid-cols-2 grid-rows-2 gap-2.5 sm:gap-3">{shapes.map(s=>{const selected=answer===s.letter;return <button key={s.letter} disabled={answered||submitting||remaining<=0} onClick={()=>submit(s.letter)} className={`group relative flex min-h-0 h-full flex-col items-center justify-center overflow-hidden rounded-[1.4rem] ${s.className} text-white shadow-lg transition-all duration-200 hover:-translate-y-1 hover:shadow-2xl active:scale-[0.96] disabled:cursor-not-allowed disabled:opacity-60 ${selected?'scale-[0.98] ring-4 ring-white/90 brightness-110 shadow-2xl':''}`}><span className="text-[clamp(3rem,8vw,6rem)] leading-none drop-shadow-md transition-transform duration-300 group-hover:scale-105">{s.symbol}</span><span className="mt-2 rounded-xl bg-black/10 px-3 py-1 text-xs font-black tracking-widest">{s.letter}</span>{selected&&<span className={`absolute right-3 top-3 rounded-full p-2 animate-in zoom-in duration-300 ${answerCorrect?'bg-white/25':'bg-white/20'}`}>{answerCorrect===false?<X className="size-4"/>:<Check className="size-4"/>}</span>}</button>})}</div><div className="mt-2 min-h-7 shrink-0 text-center">{answered?<div className={`inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-sm font-black shadow-sm animate-in zoom-in duration-300 ${answerCorrect===true?'bg-green-500/10 text-green-700 dark:text-green-300':'bg-red-500/10 text-red-700 dark:text-red-300'}`}>{answerCorrect===true?<Check className="size-4"/>:<X className="size-4"/>}{submitting?'Answer locked — checking…':answerCorrect?'Correct! Waiting for the next question.':'Not quite — waiting for the next question.'}</div>:remaining<=0?<p className="font-black text-destructive">Time's up.</p>:<p className="text-xs text-muted-foreground">Faster correct answers improve your ranking.</p>}{error&&<p className="mt-1 text-sm text-destructive">{error}</p>}</div></CardContent></Card></div></main>;
}

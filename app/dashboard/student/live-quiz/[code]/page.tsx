...
export default function LiveQuizGame(){
  ...
  useEffect(()=>{
    ...
    stateKey.current=k;
    setAnswer('');setAnswered(false);setResult(null);setResponseTime(null);setSubmitting(false);setRemaining(q.time_limit_seconds||30);setRevealedAnswers(0);
  },[quiz?.id,idx,q?.id]);

  useEffect(()=>{
    if(!answering||!q)return;
    setRevealedAnswers(1);
    const timers=[1,2,3].map(i=>window.setTimeout(()=>setRevealedAnswers(i+1),i*400));
  ...
  async function submit(v:string){
    if(!quiz||!player||!q||answered||submitting||!answering||remaining<=0||!revealedAnswers)return;
  ...
  return <main className="min-h-screen overflow-hidden bg-muted/30 px-2 py-2 sm:px-4 sm:py-4"><div className="mx-auto flex min-h-[calc(100vh-1rem)] max-w-6xl flex-col"><header className="mb-2 flex items-center justify-between rounded-2xl border bg-card px-4 py-3 shadow-sm sm:mb-3 sm:px-5"><div><p className="text-[10px] font-black uppercase tracking-[.25em] text-primary">{quiz.title}</p><p className="mt-0.5 text-sm font-black">Question {idx+1} <span className="font-medium text-muted-foreground">of {questions.length}</span></p></div><div className={`flex items-center gap-2 rounded-xl px-3 py-2 text-lg font-black tabular-nums ${remaining<=5?'bg-red-500/10 text-red-600':'bg-primary/10 text-primary'}`}><Clock className="size-5"/>{remaining}s</div></header><Card className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-[2rem] border-0 shadow-xl"><CardHeader className="px-4 pb-3 pt-6 text-center sm:px-8 sm:pt-8"><div className={`mx-auto w-fit rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider ${answered?'bg-green-500/10 text-green-600':'bg-primary/10 text-primary'}`}>{answered?'Answer submitted':revealedAnswers<4?'Answers are appearing…':'Choose an answer'}</div><CardTitle className="mx-auto mt-3 max-w-4xl text-2xl font-black leading-tight sm:text-4xl">{q.question}</CardTitle><div className="mx-auto mt-4 h-2 w-full max-w-3xl overflow-hidden rounded-full bg-muted"><div className={`h-full transition-[width] duration-100 ${remaining<=5?'bg-red-500':'bg-primary'}`} style={{width:`${pct}%`}}/></div></CardHeader><CardContent className="grid min-h-0 flex-1 grid-cols-2 grid-rows-2 gap-2 p-2 sm:gap-4 sm:p-5">{shapes.map((s,i)=>{const selected=answer===s.letter,revealed=i<revealedAnswers;return <button key={s.letter} disabled={!revealed||answered||submitting||remaining<=0} onClick={()=>submit(s.letter)} className={`group relative flex min-h-32 h-full flex-col items-center justify-center overflow-hidden rounded-[1.5rem] ${s.className} text-white shadow-lg transition duration-150 hover:-translate-y-1 hover:shadow-xl active:scale-[.98] disabled:cursor-default disabled:opacity-70 ${revealed?'animate-in fade-in zoom-in-95 duration-300':'opacity-0'} ${selected?'ring-4 ring-white/90 brightness-110':''}`}><span className="absolute right-4 top-3 text-lg font-black opacity-50">{s.letter}</span><Shape type={s.shape}/><span className="mt-2 text-xs font-black uppercase tracking-widest opacity-80">{selected?'Selected':s.letter}</span>{selected&&<Check className="absolute bottom-3 right-3 size-6"/>}</button>})}</CardContent><div className="px-5 pb-4 text-center text-xs font-medium text-muted-foreground sm:pb-5">{answered?'Answer locked in • Wait for the result':revealedAnswers<4?'Watch the choices appear…':'Select one answer — you can only submit once'}</div></Card></div></main>;
}
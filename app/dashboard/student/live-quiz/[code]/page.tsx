...
export default function LiveQuizGame(){
  ...
  if(!quiz)return <main className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top,hsl(var(--primary)/.12),transparent_50%)]"><div className="flex items-center gap-3 rounded-2xl border bg-card px-5 py-4 font-semibold text-muted-foreground shadow-lg"><Loader2 className="size-5 animate-spin text-primary"/>Connecting to live quiz…</div></main>;
  if(!player)return <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_top,hsl(var(--primary)/.2),transparent_48%)] px-5"><Card className="w-full max-w-md overflow-hidden rounded-[2rem] border-0 shadow-2xl"><div className="bg-primary p-9 text-center text-primary-foreground"><Zap className="mx-auto size-9"/><p className="mt-4 text-[11px] font-black uppercase tracking-[0.3em]">Live Quiz</p><h1 className="mt-2 text-3xl font-black">{quiz.title}</h1></div><CardContent className="space-y-5 p-7 text-center"><div className="rounded-2xl border bg-muted/30 p-5"><p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Playing as</p><p className="mt-1 text-xl font-black">{nickname}</p></div><Button size="lg" className="h-13 w-full rounded-2xl font-black transition-transform active:scale-[.98]" onClick={join} disabled={joining}>{joining?<><Loader2 className="mr-2 size-5 animate-spin"/>Joining…</>:<>Join Lobby <Zap className="ml-2 size-5"/></>}</Button>{error&&<p className="text-sm text-destructive">{error}</p>}</CardContent></Card></main>;
  if(quiz.status==='lobby')return <main className="relative flex min-h-screen items-center justify-center overflow-hidden px-5"><div className="w-full max-w-xl text-center"><div className="mx-auto flex size-24 items-center justify-center rounded-[2rem] bg-primary text-primary-foreground shadow-2xl animate-[pulse_2.5s_ease-in-out_infinite]"><Wifi className="size-10"/></div><p className="mt-7 text-[11px] font-black uppercase tracking-[0.35em] text-primary">Connected to host</p><h1 className="mt-2 text-5xl font-black">You're in!</h1><p className="mt-4 text-muted-foreground">Keep your eyes on the teacher's screen. Your device is your answer controller.</p><div className="mx-auto mt-8 rounded-[2rem] border bg-card p-6 shadow-xl"><div className="flex items-center justify-center gap-2"><span className="size-2.5 animate-pulse rounded-full bg-green-500"/><span className="font-bold">Waiting for the host to start</span></div><div className="mt-6 grid grid-cols-4 gap-2.5">{shapes.map(s=><div key={s.letter} className={`flex h-16 items-center justify-center rounded-2xl ${s.className} text-3xl font-black text-white transition-transform duration-300 hover:-translate-y-1`}>{s.symbol}</div>)}</div></div></div></main>;
  if(quiz.status==='finished')return <main className="min-h-screen px-4 py-8"><div className="mx-auto max-w-2xl"><Card className="overflow-hidden rounded-[2rem] border-0 shadow-2xl"><CardHeader className="bg-primary/5 p-10 text-center"><Trophy className="mx-auto size-16 text-primary"/><p className="mt-5 text-[11px] font-black uppercase tracking-[0.3em] text-primary">Game complete</p><CardTitle className="mt-2 text-4xl font-black">Quiz Finished!</CardTitle><p className="mt-3 text-muted-foreground"><b className="text-foreground">{player.correct_answers}</b> correct</p></CardHeader><CardContent className="p-5"><div className="space-y-2">{ranking.map((p,i)=><div key={p.id} className={`flex items-center gap-3 rounded-2xl border p-4 transition-all duration-300 ${p.id===player.id?'border-primary bg-primary/10 shadow-sm':''}`}><span className="flex size-9 items-center justify-center rounded-xl bg-muted font-black">{i+1}</span><span className="flex-1 truncate font-bold">{p.nickname}{p.id===player.id&&<span className="ml-2 text-xs text-primary">YOU</span>}</span><span className="text-sm font-black">{p.correct_answers} correct</span></div>)}</div><Button className="mt-6 w-full rounded-xl" onClick={()=>router.push('/dashboard/student')}><ArrowLeft className="mr-2 size-4"/>Back to Student Dashboard</Button></CardContent></Card></div></main>;

...
export default function TestReviewPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (loadedTest.allow_review === false) {
          throw new Error('Review is not permitted for this test.');
        }

        const submissionResponse = await fetch(`${url}/rest/v1/test_submissions?test_id=eq.${encodeURIComponent(id)}&student_id=eq.${encodeURIComponent(studentId)}&select=*&order=created_at.desc`, { headers, cache: 'no-store' });
  ...
  if (error || !test || !submission) {
    return <div className="min-h-screen bg-background"><main className="mx-auto max-w-xl px-6 py-12"><Card><CardContent className="py-10 text-center"><p className="text-destructive">{error || 'Review unavailable.'}</p><Button className="mt-4" onClick={() => router.push('/dashboard/student')}>Back to Dashboard</Button></CardContent></Card></main></div>;
  }
      ...
      <main className="mx-auto max-w-4xl space-y-6 px-6 py-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-muted-foreground">{test.title}</p>
            <p className="mt-2 text-sm font-medium">Score: {Number(submission.score).toFixed(2)}/100 · Attempts used: {Math.max(1, Number(test.max_attempts) || 1)}/{Math.max(1, Number(test.max_attempts) || 1)}</p>
          </div>
        ...
        <div className="space-y-4">
          {questions.map((question, index) => {
            ...
            return (
              <Card key={question.id} className={correct ? 'border-green-500' : 'border-red-500'}>
                <CardHeader>
                  <div className="flex items-start justify-between gap-4">
                    <CardTitle className="text-lg">{index + 1}. {question.question}</CardTitle>
                    <span className="shrink-0 rounded-full bg-muted px-3 py-1 text-xs font-semibold">{question.points} pts</span>
                  </div>
                  <p className={`flex items-center gap-2 text-sm font-semibold ${correct ? 'text-green-600' : 'text-red-600'}`}>
                    {correct ? <Check className="size-4" /> : <X className="size-4" />}
                    {correct ? 'Correct' : 'Incorrect'}
                  </p>
                </CardHeader>
                <CardContent className="space-y-3">
                  {type === 'multiple-choice' && (['A', 'B', 'C', 'D'] as const).map(letter => {
                    const text = question[`option_${letter.toLowerCase()}` as 'option_a' | 'option_b' | 'option_c' | 'option_d'];
                    const selectedAnswer = selected === letter;
                    const correctAnswer = question.correct_answer === letter;

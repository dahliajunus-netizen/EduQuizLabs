...
export default function TeacherLiveQuiz() {
  ...
  useEffect(() => {
    ...
    if (quiz.status === 'intermission') { const t = window.setTimeout(() => void next(), 5000); return () => window.clearTimeout(t); }
  }, [quiz?.id, quiz?.status, quiz?.current_question, quiz?.question_started_at, currentQ?.id]);

  if (!quiz) return <main className="min-h-screen bg-muted/30 px-4 py-8"><div className="mx-auto max-w-6xl"><div className="flex items-center gap-3"><Button variant="ghost" onClick={() => router.push('/dashboard/teacher')}><ArrowLeft className="size-4" />Back</Button><div><h1 className="text-3xl font-black tracking-tight">Live Quiz</h1><p className="text-sm text-muted-foreground">Create a fast-paced quiz for your class.</p></div></div><div className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
    <Card className="rounded-3xl"><CardContent className="space-y-4 p-6"><div className="flex items-center gap-2 text-sm font-bold"><Sparkles className="size-4 text-primary" />Create a live quiz</div><Input value={title} onChange={e => setTitle(e.target.value)} placeholder="Quiz title" /><select value={classCode} onChange={e => setClassCode(e.target.value)} className="h-11 w-full rounded-xl border bg-background px-3 text-sm"> <option value="">Choose class</option>{classes.map(c => <option key={c.code} value={c.code}>{c.class_name}{c.school_name ? ` — ${c.school_name}` : ''}</option>)}</select><Button onClick={createTemplate} disabled={loading} className="w-full rounded-xl">{loading ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Plus className="mr-2 size-4" />}Create quiz</Button>{error && <p className="text-sm text-destructive">{error}</p>}</CardContent></Card>
  ...
  return <main className="min-h-screen bg-muted/30 px-4 py-8 sm:py-10"><div className="mx-auto max-w-6xl"><header className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.3em] text-primary">{quiz.title}</p><h1 className="mt-1 text-3xl font-black">Question {current+1} <span className="text-muted-foreground">/ {questions.length}</span></h1></div><div className="flex items-center gap-2 rounded-2xl border bg-card px-4 py-2 text-sm font-black"><Users className="size-4 text-primary" />{players.length} players</div></header>{isIntermission ? <div className="flex min-h-[72vh] items-center justify-center"><div className="text-center"><div className="mx-auto flex size-28 animate-pulse items-center justify-center rounded-full bg-primary text-primary-foreground shadow-2xl"><ChevronRight className="size-14" /></div><p className="mt-8 text-xs font-black uppercase tracking-[.3em] text-primary">Intermission</p><h1 className="mt-2 text-5xl font-black tracking-tight sm:text-7xl">Next up</h1><p className="mt-3 text-muted-foreground">Get ready for the next question.</p><p className="mt-2 text-xs font-bold text-muted-foreground/70">Next question in 5 seconds</p></div></div> : <><Card className="mt-7 rounded-[2rem] border-0 shadow-xl"><CardContent className="p-7 sm:p-10"><div className="flex items-center justify-between"><span className="rounded-full bg-primary/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-primary">{isReveal?'Question reveal':isAnswering?'Answering now':isResults?'Results':'Live'}</span>{isAnswering && <span className="text-lg font-black tabular-nums">{remaining}s</span>}</div><h2 className="mt-6 text-center text-3xl font-black leading-tight sm:text-5xl">{currentQ?.question}</h2></CardContent></Card>{(isAnswering||isResults||isReveal)&&<div className="mt-6 grid items-stretch gap-3 sm:grid-cols-2">{options.map(o=><div key={o.letter} className={`relative flex min-h-36 items-center overflow-hidden rounded-2xl border p-4 transition-colors duration-300 sm:min-h-40 ${isResults&&currentQ?.correct_answer===o.letter?o.soft:'bg-card'}`}><OptionIcon opt={o}/><div className="ml-4 min-w-0"><p className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Answer {o.letter}</p><p className="mt-1 text-base font-black sm:text-lg">{currentQ?.[o.key]}</p></div>{isResults&&<span className="absolute right-4 top-4 text-xs font-black">{answerCounts[o.letter]||0}</span>}</div>)}</div>}{isReveal&&<p className="mt-8 text-center text-xs font-bold uppercase tracking-[.2em] text-muted-foreground">Answers are being revealed…</p>}{isAnswering&&<div className="mt-6 rounded-2xl border bg-card p-5"><div className="flex items-center justify-between text-sm font-bold"><span>Responses</span><span>{totalVotes} submitted</span></div><div className="mt-3 h-3 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all" style={{width:`${players.length?Math.min(100,(totalVotes/players.length)*100):0}%`}}/></div></div>}{isResults&&<div className="mt-6 rounded-2xl border bg-card p-5"><div className="flex items-center justify-between"><span className="font-black">Results</span><span className="text-sm font-bold text-muted-foreground">Next question in 4 seconds</span></div></div>}</>}</div></main>;
}

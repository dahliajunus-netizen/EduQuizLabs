...
import { ArrowLeft, Check, ChevronRight, Clock3, Copy, Crown, Loader2, Play, Plus, Radio, Sparkles, Trash2, Trophy, Users, X } from 'lucide-react';
...
export default function TeacherLiveQuiz() {
  ...
  async function beginAnswering() { if (!quiz) return; try { const r = await rpc('live_quiz_begin_answering', { p_quiz_id: quiz.id }); setQuiz(r?.[0] || r || { ...quiz, status: 'answering', question_started_at: new Date().toISOString() }); setAnswerCounts({ A: 0, B: 0, C: 0, D: 0 }); } catch (e) { setError(e instanceof Error ? e.message : 'Could not open answers.'); } }
  async function showResults() { if (!quiz) return; try { const r = await rpc('live_quiz_begin_results', { p_quiz_id: quiz.id }); setQuiz(r?.[0] || r || { ...quiz, status: 'results' }); } catch (e) { setError(e instanceof Error ? e.message : 'Could not reveal results.'); } }
  ...
  if (!quiz) return <main className="min-h-screen bg-muted/30 px-4 py-8"><div className="mx-auto max-w-6xl"><div className="flex items-center gap-3"><Button variant="ghost" onClick={() => router.push('/dashboard/teacher')}><ArrowLeft className="mr-2 size-4" />Back</Button><div><p className="text-xs font-black uppercase tracking-[.3em] text-primary">Live Quiz</p><h1 className="text-3xl font-black">Host a fast-paced quiz</h1></div></div><div className="mt-7 grid gap-6 lg:grid-cols-2"><Card className="rounded-3xl"><CardContent className="space-y-4 p-6"><div className="flex items-center gap-2 text-sm font-bold"><Sparkles className="size-4 text-primary" />Create a live quiz</div><Input value={title} onChange={e => setTitle(e.target.value)} placeholder="Quiz title" /><select value={classCode} onChange={e => setClassCode(e.target.value)} className="h-11 w-full rounded-xl border bg-background px-3 text-sm"><option value="">Choose class</option>{classes.map(c => <option key={c.code} value={c.code}>{c.class_name}{c.school_name ? ` — ${c.school_name}` : ''}</option>)}</select><Button onClick={createTemplate} disabled={loading} className="w-full rounded-xl">{loading ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Plus className="mr-2 size-4" />}Create quiz</Button>{error && <p className="text-sm text-destructive">{error}</p>}</CardContent></Card><Card className="rounded-3xl"><CardContent className="p-6"><h2 className="font-black">Your saved quizzes</h2><div className="mt-4 space-y-2">{templates.length ? templates.map(t => <button key={t.id} onClick={() => void openTemplate(t)} className="flex w-full items-center justify-between rounded-2xl border p-4 text-left hover:bg-muted/50"><span><b>{t.title}</b><span className="mt-1 block text-xs text-muted-foreground">{t.class_code}</span></span><ChevronRight className="size-4 text-muted-foreground" /></button>) : <p className="text-sm text-muted-foreground">No saved quizzes yet.</p>}</div></CardContent></Card></div>{selected && <Card className="mt-6 rounded-3xl"><CardContent className="space-y-5 p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-primary">Editing</p><h2 className="text-2xl font-black">{selected.title}</h2></div><Button variant="outline" onClick={() => setSelected(null)}>Close</Button></div><div className="grid gap-4 md:grid-cols-2"><Input value={qText} onChange={e => setQText(e.target.value)} placeholder="Question" /><div className="grid grid-cols-2 gap-2"><Input value={a} onChange={e => setA(e.target.value)} placeholder="Answer A" /><Input value={b} onChange={e => setB(e.target.value)} placeholder="Answer B" /><Input value={c} onChange={e => setC(e.target.value)} placeholder="Answer C" /><Input value={d} onChange={e => setD(e.target.value)} placeholder="Answer D" /></div></div><div className="flex flex-wrap items-center gap-3"><select value={correct} onChange={e => setCorrect(e.target.value)} className="h-10 rounded-xl border bg-background px-3 text-sm"><option>A</option><option>B</option><option>C</option><option>D</option></select><Button onClick={addQuestion} className="rounded-xl"><Plus className="mr-2 size-4" />Add question</Button><Button onClick={() => void launch()} disabled={!questions.length || loading} className="rounded-xl"><Play className="mr-2 size-4" />Launch live quiz</Button></div><div className="space-y-2">{questions.map((item,i)=><div key={item.id} className="flex items-center justify-between rounded-2xl border p-4"><div><b>Question {i+1}</b><p className="text-sm text-muted-foreground">{item.question}</p></div><Button variant="ghost" size="icon" onClick={() => void delQuestion(item)}><Trash2 className="size-4" /></Button></div>)}</div></CardContent></Card>}</div></main>;
  ...
  return <main className="min-h-screen bg-muted/30 px-3 py-5 sm:px-6 sm:py-7"><div className="mx-auto max-w-screen-2xl">
    ...
    {isLobby && <div className="grid gap-6 lg:grid-cols-[1fr_360px]"><Card className="overflow-hidden rounded-[2rem] border-0 shadow-sm"><CardContent className="relative flex min-h-[540px] flex-col items-center justify-center overflow-hidden p-6 text-center sm:p-10"><div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_20%,hsl(var(--primary)/.16),transparent_55%)]" /><div className="relative"><div className="mx-auto mb-5 flex size-16 items-center justify-center rounded-3xl bg-primary text-primary-foreground shadow-lg"><Users className="size-8" /></div><p className="text-xs font-black uppercase tracking-[.25em] text-primary">Waiting room</p><h1 className="mt-3 text-4xl font-black tracking-tight sm:text-6xl">Join the game</h1><p className="mx-auto mt-3 max-w-md text-sm text-muted-foreground">Students can scan the QR code or use the game code below.</p><div className="mx-auto mt-7 flex flex-col items-center gap-5 sm:flex-row sm:items-stretch"><div className="rounded-3xl border bg-white p-4 shadow-lg"><QRCodeSVG value={joinUrl || 'https://eduquizlabs.com/join'} size={210} bgColor="#ffffff" fgColor="#111827" level="H" includeMargin /></div><div className="flex min-w-[220px] flex-col justify-center rounded-3xl border bg-background px-5 py-4 text-left shadow-lg"><p className="text-[10px] font-black uppercase tracking-[.3em] text-muted-foreground">Game code</p><p className="mt-1 font-mono text-4xl font-black tracking-[.22em]">{quiz.game_code}</p><Button variant="outline" onClick={() => { void navigator.clipboard?.writeText(quiz.game_code || ''); setCopied(true); window.setTimeout(() => setCopied(false), 1400); }} className="mt-4 rounded-xl font-bold">{copied ? <Check className="mr-2 size-4 text-green-500" /> : <Copy className="mr-2 size-4" />}{copied ? 'Copied' : 'Copy code'}</Button></div></div><p className="mt-4 text-xs font-semibold text-muted-foreground">Scan → enter nickname → Join</p><Button onClick={() => void next()} disabled={!players.length} className="mt-7 h-12 rounded-xl px-7 font-black shadow-md"><Play className="mr-2 size-4 fill-current" /> Start quiz</Button>{!players.length && <p className="mt-3 text-xs font-semibold text-muted-foreground">Waiting for the first player…</p>}</div></CardContent></Card><Card className="rounded-[2rem] border-0 shadow-sm"><CardContent className="p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><h2 className="font-black">Players</h2><p className="text-xs text-muted-foreground">{players.length} joined</p></div><div className="flex size-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><Users className="size-5" /></div></div>{players.length === 0 ? <div className="flex min-h-72 items-center justify-center rounded-2xl bg-muted/40 text-center"><div><Users className="mx-auto mb-3 size-7 text-muted-foreground/40" /><p className="font-bold">Nobody yet</p><p className="mt-1 text-xs text-muted-foreground">Keep this screen open while students join.</p></div></div> : <div className="max-h-[430px] space-y-2 overflow-auto">{players.map((p, i) => <div key={p.id} className="flex items-center gap-3 rounded-2xl border bg-background p-3"><div className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-muted text-xs font-black">{i + 1}</div><span className="min-w-0 flex-1 truncate font-bold">{p.nickname}</span><span className="text-xs font-black text-muted-foreground">{p.score}</span></div>)}</div>}</CardContent></Card></div>}

    {isIntermission && <div className="flex min-h-[78vh] items-center justify-center"><div className="text-center"><div className="mx-auto flex size-28 animate-pulse items-center justify-center rounded-full bg-primary text-primary-foreground shadow-2xl"><ChevronRight className="size-14" /></div><p className="mt-8 text-xs font-black uppercase tracking-[.3em] text-primary">Intermission</p><h1 className="mt-2 text-5xl font-black tracking-tight sm:text-7xl">Next up</h1><p className="mt-3 text-muted-foreground">Get ready for the next question.</p><p className="mt-2 text-xs font-bold text-muted-foreground/70">Next question in 5 seconds</p></div></div>}

    {(isReveal || isAnswering || isResults) && currentQ && <><header className="flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.3em] text-primary">{quiz.title}</p><h1 className="mt-1 text-2xl font-black sm:text-3xl">Question {current+1} <span className="text-muted-foreground">/ {questions.length}</span></h1></div>{isAnswering && <div className="rounded-2xl border bg-card px-5 py-2.5 text-xl font-black tabular-nums shadow-sm">{remaining}s</div>}</header><Card className="mt-5 rounded-[1.5rem] border-0 shadow-xl"><CardContent className="p-5 sm:p-7"><div className="flex items-center justify-between"><span className="rounded-full bg-primary/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-primary">{isReveal?'Question reveal':isAnswering?'Answering now':'Results'}</span></div><h2 className="mt-4 text-center text-2xl font-black leading-tight sm:text-4xl">{currentQ.question}</h2></CardContent></Card>{isReveal&&<p className="mt-8 text-center text-sm font-bold text-muted-foreground">Answers are being revealed…</p>}{(isAnswering||isResults)&&<div className="mt-5 grid min-h-[68vh] grid-cols-1 gap-4 sm:grid-cols-2 sm:grid-rows-2 sm:gap-5">{options.map(o=>{const count=answerCounts[o.letter]||0;const pct=totalVotes?Math.round((count/totalVotes)*100):0;const correctAnswer=currentQ.correct_answer===o.letter;return <div key={o.letter} className={`relative flex min-h-56 items-center overflow-hidden rounded-[1.75rem] border p-7 transition-colors sm:min-h-0 sm:p-10 ${isResults&&correctAnswer?o.soft:'bg-card'}`}><OptionIcon opt={o}/><div className="ml-5 min-w-0"><p className="text-xs font-black uppercase tracking-wider text-muted-foreground">Answer {o.letter}</p><p className="mt-2 text-xl font-black leading-snug sm:text-3xl lg:text-4xl">{currentQ[o.key]}</p></div>{isResults&&<div className="absolute right-7 top-7 text-base font-black">{count} · {pct}%</div>}</div>})}</div>}{isResults&&<p className="mt-4 text-center text-xs font-bold uppercase tracking-[.2em] text-muted-foreground">Next question in 4 seconds</p>}</>}
  </div></main>;
}

    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto space-y-6 px-6 py-8">
        ...
        {loading ? <Loader2 className="mx-auto size-8 animate-spin" /> : tests.map(t => (
          <Card key={t.id}>
            <CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle>{t.title}</CardTitle><p className="text-sm text-muted-foreground">Class: {t.class_code} · {questions[t.id]?.length || 0} questions</p><div className="mt-2 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-muted px-2.5 py-1">Due: {formatDueDate(t.due_date)}</span>{t.time_limit_minutes ? <span className="rounded-full bg-muted px-2.5 py-1">Time: {t.time_limit_minutes} min</span> : null}{t.test_password ? <span className="rounded-full bg-muted px-2.5 py-1">Password protected</span> : null}</div></div><div className="flex gap-2"><Button variant="outline" onClick={() => void togglePublished(t)}>{t.published ? <><EyeOff className="mr-2 size-4" />Unpublish</> : <><Eye className="mr-2 size-4" />Publish</>}</Button><Button variant="destructive" onClick={() => void deleteTest(t.id)}><Trash2 className="size-4" /></Button></div></div></CardHeader>

...
export function LoginForm() {
    ...
    try {
      ...
      if (!authResponse.ok || !authData.access_token || !authData.user?.id) {
        ...
        if (lower.includes('email not confirmed') || lower.includes('confirm your email')) {
          ...
          setError('Supabase rejected these credentials. Double-check the email and password, or use “Forgot password?” to set a new password.');
        } else if (message) {
      ...
      if (!profile) {
        ...
        setError('Your email and password are correct, but your profile could not be loaded. Please check the profile trigger/database setup.');
        return;
      }
  ...
  return (
    <div className="relative flex flex-col gap-6">
      ...
      {error && <div role="alert" className="break-words rounded-md bg-red-500/10 p-3 text-sm font-medium text-red-500">{error}</div>}

-- Fix test retries and enforce attempt limits server-side.
-- Run/apply this migration in Supabase before using Retry Assessment.

-- The old unique index allowed only one submission per student/test,
-- which conflicts with tests.max_attempts.
...
begin
  ...
  -- Finish the student's latest in-progress attempt, if one exists.
  ...
  if v_started_at is not null
     ...
     and now() > v_started_at + make_interval(mins => v_test.time_limit_minutes) then
    -- Timed-out attempts are still submitted; unanswered questions score zero.
    null;
  end if;
  ...
  insert into public.test_submissions (
    test_id, student_id, score, submitted_at, answers
  ) values (
    p_test_id, p_student_id, v_score, now(), coalesce(p_answers, '{}'::jsonb)

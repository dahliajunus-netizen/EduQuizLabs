...
begin
  ...
  select count(*)::integer into v_used_attempts from public.test_submissions where test_id = p_test_id and student_id = p_student_id;
  if v_used_attempts >= v_max_attempts then raise exception 'You have used all available attempts for this test.'; end if;
  ...
  for v_question in select id, correct_answer, option_a, points, question_type from public.test_questions where test_id = p_test_id order by question_order asc, id asc loop
    ...
    if v_question_type in ('fill_blank', 'fill_in_blank') then
      if lower(trim(v_answer)) = lower(trim(coalesce(v_question.option_a, ''))) then v_earned := v_earned + greatest(coalesce(v_question.points, 1), 0); end if;
    elsif v_question_type = 'matching' then
      begin
        v_expected := coalesce(v_question.option_a, '[]')::jsonb;
        v_selected := coalesce(v_answer, '{}')::jsonb;
        if jsonb_typeof(v_expected) = 'array' and jsonb_array_length(v_expected) > 0 and not exists (select 1 from jsonb_array_elements(v_expected) p where coalesce(v_selected ->> (p ->> 'left'), '') <> coalesce(p ->> 'right', '')) then
      ...
      if upper(trim(v_answer)) = upper(trim(coalesce(v_question.correct_answer, ''))) then v_earned := v_earned + greatest(coalesce(v_question.points, 1), 0); end if;
  ...
  if v_attempt.id is not null then
    update public.test_attempts set answers = coalesce(p_answers, '{}'::jsonb), status = 'completed', updated_at = now(), completed_at = now() where id = v_attempt.id;
  end if;
